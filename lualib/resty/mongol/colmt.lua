...
function colmethods:find(query, returnfields, num_each_query, opt)
    num_each_query = num_each_query or 100
    if num_each_query == 1 then
        return nil, "num_each_query must larger than 1"
    end
    return new_cursor(self, query, returnfields, num_each_query, opt)
end

function colmethods:find_one(query, returnfields, opt)
    local id, results, t = self:query(query, returnfields, 0, 1, opt)
